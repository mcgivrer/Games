<?php
class DataRS extends TData implements IData{
	...
	private function transformToRS($ret) {
		$rs = new ResultSet();
		foreach($ret as $item){
			$rs->add($item);
		}
	...
	private function manageOptions($items, $options){
		...
		if($options!=null){
			...
			if(isset($options['limit'])){
				...
				$arrayOffset =(isset($items['array_offset'])?$items['array_offset']:0);
				if($count>$options['limit']){
					$itemsOptionized = array_slice($items-getData(), 0, $options['limit'],true);
				} 
				$itemsOptionized ->setMeta('offset', $arrayOffset);
				$itemsOptionized ->setMeta('pagesize', $options['limit']);
			}
		}
		$itemsOptionized ->setMeta('count',$count);
	 ...
	 */
	public function getDataListDistinct(
				$entityName,
				$distinctOnAttribute,
				$attributes=null,
				$options=null){
